...
export default async function BreakingPage() {
  ...
  // Get all breaking summaries from last 48h — no topic filter, all topics
  const { data: breakingSummaries } = await supabase
    .from('article_summaries')
    .select('article_id')
    .eq('is_breaking', true)
    .gte('created_at', EIGHT_HOURS_AGO())
    ...
    return (
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="flex flex-col items-center justify-center h-64 text-center">
          ...
          <p className="text-muted text-sm">No breaking news in the last 8 hours.</p>
        </div>
      </div>
    )
  }
  ...
  const [{ data: articles }, { data: summaries }, { data: crossFlags }, { data: articleTopicRows }, { data: sourceRatingRows }] =
    await Promise.all([
      supabase.from('articles').select('*').in('id', articleIds).gte('published_at', EIGHT_HOURS_AGO()),
    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-muted text-sm mt-1.5">
          {feed.length} significant {feed.length === 1 ? 'story' : 'stories'} · last 8 hours
        </p>
      </div>
